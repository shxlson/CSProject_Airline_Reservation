...
def Create_Database_Airline():
    ...
    s1="create database airlines"
    cursor.execute(s1)
Create_Database_Airline()

'''table creation airline'''

def table_creation_airline():
    import mysql.connector
    mycon=mysql.connector.connect(host='localhost',user='root',passwd='1234',database='airlines')
    cursor=mycon.cursor()
    mycon.autocommit=True
    s1="create table airline(name varchar(100),phno varchar(15)  primary key,age int(4),gender varchar(50),from_f varchar(100),to_t varchar(100),date_d varchar(20))"
    cursor.execute(s1)
table_creation_airline()


'''table creation user accounts'''
...
def menu():
    ...
    while ch==1:
        print('WELECOME TO ONLINE AIRLINE TICKET RESERVATION SYSTEM') 
        print('1.SIGN IN')
        print('2.SIGN UP')
...
def ticket_booking():
    ...
    v=a[Gender]
    s1="insert into airline values('{}',{},{},'{}','{}','{}','{}')".format(nm,phno,age,v,fr,to,date)
    cursor.execute(s1)
...
def ticket_checking():
    ...
    if ch==1:
        ...
        try:
            s1="select * from airline where phno=phno"
            cursor.execute(s1)
...
def ticket_cancelling():
    ...
    if ch==1:
        phno=input('Enter your phone number:')
        s1="delete from airline where phno=phno"
        cursor.execute(s1)
